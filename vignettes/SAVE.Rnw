...
sheets of metal of a particular thickness (\code{G}) are compressed by
two 
electrodes under a certain 
applied load (\code{L}). Electric current of magnitude \code{C} is
passed through 
...
scientists are interested in the diameter of this nugget (\code{N}). 
...
\code{t}, which is a calibration input related to contact resistance. 
...
\code{mean.formula=as.formula("~1")}) and
...
parameter \code{t}, and also options pertaining to the MCMC
...
posterior distribution of the calibration parameter \code{t} and of the
...
diameter \code{N} 
varies with the current \code{C}. One might assess this variation by
looking at the derivative of \code{N} with respect to \code{C}. Let's
do that for \code{G}=1 and \code{L}=4. We start by predicting reality
at an equally-spaced grid of \code{C} values between 20 and 30:
\begin{CodeInput}
...
\begin{figure}[h]
  ...
  \caption{Bias-corrected prediction of the derivative of \code{N}
    with respect to \code{C} --- the estimate is the solid line; the
...
\code{t}. This allows us to produce the pure-model estimate of the
derivative:
\begin{CodeInput}
R> u <- 3.2; load <- 4; g <- 1; 
