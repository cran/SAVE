...
%almost as usual
\author{Jes\'us Palomo\\URJC \And 
        Rui Paulo\\ISEG and CEMAPRE \And
        Gonzalo Garc\'\i a-Donato\\UCLM}
\title{\pkg{SAVE}: an \proglang{R} Package for the Statistical
...
%% comma-separated 
\Plaintitle{\pkg{SAVE}: an R package for the Statistical Analysis of
...
\Abstract{
  ...
  use with a real data example and in the context of a simulated
  example.
}
...
the suite of \pkg{Dice} packages: \pkg{DiceKriging} and 
\pkg{DiceOptim} \citep{dice12}, \pkg{DiceEval} \citep{DE} and
\pkg{Dice\-De\-sign} \citep{DD}. \pkg{DiceKriging}
...
dedicated to optimization of complex computer models based on
kriging models. 
\pkg{DiceDesign} 
facilitates the construction of various space-filling designs
...
goals. However, it
implements the metho\-do\-lo\-gy in \citet{Kenn:OHag:2001} 
...
on analytical or on numerical integration. As a consequence, with
\pkg{SAVE} one can for instance explore the posterior 
...
describe the statistical methodology. In Section~\ref{overview}, we
describe the 
structure of the package, and in
Section~\ref{spotweld} we illustrate its use in the context of a real
example. In Section~\ref{synth}, we use it in a situation where the
truth is known. Technical details are, whenever possible, relegated to the
appendices. 
...
$\n v$ is a vector of unknown calibration and/or tuning parameters in
the model. We have
access to the data obtained by evaluating the computer model at a
design set 
consisting of $N$ points $D^M=\{(\n x_1,\n v_1),\ldots,(\n x_{N},\n
v_{N})\}$. % Model output and associated design are stored in a {\tt
...
We denote by
$\n y^M=(y^M(\n x_1,\n v_1),\ldots,y^M(\n x_{N},\n
v_{N}))$ the vector of model evaluations, and we refer to these data
as computer model data, or simply as model data.
...
estimates of the output at untested configu\-rations along with an
...
using the \proglang{R} package \pkg{DiceKriging}
\citep{dice12}. 
% , so that
% $$
% y^M (\n x, \n u)\mid \n y^M, \hat{\n \theta}^M, \hat{\n \theta}^L \sim
...
${\n\theta}^L$ and ${\n\theta}^M$ which specify, respectively, the
linear trend and the covariance kernel of the Gaussian process prior. In practical
terms, the output of 
the computer model 
at a set of untested configurations (given $\n y^M$ and the parameter
estimates) follows a multivariate normal
distribution with known mean vector and covariance matrix. 
%The
...
\begin{equation}
y_k^F(\n x_j^{\star}) = y^R(\n x_j^{\star})+\varepsilon_{kj},\quad j=1,\ldots,n;\
k=1,\ldots, n_j 
\end{equation}
where $y^R(\cdot)$ represents the real process and $\varepsilon_{kj}$
...
the same configuration of the controllable inputs. We denote the
complete set of field observations by $\n y^F$.
...
u$ is the unknown value of the calibration vector $\n v$ which we are
ultimately interested in estimating. In Figure~\ref{diag1} we have
included a diagram which schematically characterizes the framework we
have just described. 
...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.1]{FunctionalDiagram2_letters}
  \caption{Schematic representation of the framework.}\label{diag1}
\end{figure}
...
% The \ourpack\ function that implements this
% task is called \code{bayesfit}.

Once the posterior distribution of all the unknowns in the statistical
model is
obtained, we can proceed to the validation step. For that, we need to
select a set of configurations for the controllable inputs, $D^V$ ---
e.g., $D^V=\{\n x_{1}^{\dagger},\ldots,\n x_r^{\dagger}\}$ --- at which
we are interested in assessing the accuracy of the computer
model. For a particular value $\n u$ for the calibration parameter, let 
$D_{\scriptsize \n u}^V=\{(\n x_{1}^{\dagger},\n u),\ldots,(\n
x_r^{\dagger},\n u)\}$, i.e., the design that results from augmenting
each of the configurations in $D^V$ with the vector $\n u$ for the
calibration parameters. In general, we denote by $h(D)$ the
vector that results from evaluating the function $h$ at the elements
of $D$.
% To 
% address the validation step, w
We must obtain draws from the distribution
\begin{align}\label{real}
% f(y^M(D_{\n {\scriptsize u}^\star}^V), b(D^V)&\mid \n y^M, \n y^F)=\\
% =&
\int 
f(y^M(D_{\scriptsize \n u}^V),b(D^V)\mid \n y^M, \n y^F, \n u,\n
\theta^F)\ \pi( \n 
...
sample drawn from the posterior distribution. [The distribution of 
$y^M(D_{\scriptsize \n u}^V),b(D^V)\mid \n y^M, \n y^F, \n u,\n
\theta^F$ is
multivariate normal, cf. \citet{Baya:2007} for further details.] 
% The
% \ourpack\ function 
...
\item The pure-model prediction of reality at $D^V$ is obtained by
  ...
  or by exercising the emulator. 
% Function \code{predictcode} obtains
%   draws from the emulator, but also returns
  The mean vector of the
  emulator (formula \eqref{meanapp}, in Appendix~\ref{app.emu}, with
  unknown parameters replaced  
  by estimates), can then be used as an estimate of the output of the
  model. Denote the pure-model prediction of reality by $\hat{\n y}^M$.
\item The tolerance bars measuring  the accuracy of $\hat{\n y}^M$ as a
...
\item It is also possible to estimate the bias associated with the
  pure-model prediction, $\n b_{\hat{\n u}}=y^R(D^V)-\hat{\n y}^M$:
    ...
    point estimate is $\hat{\n b}_{\hat{\n u}}=\hat{\n y}^R-\hat{\n y}^M$
      and $1-\gamma$ pointwise credible intervals can be determined by
...
% tolerance bounds and estimated bias func\-tion is called \code{validate}.
...
the problem where the response is the variable \code{expand};
...
If in order to obtain the field data
the controllable inputs (if any are present) have not been varied but have instead been fixed at a single 
value, you have to specify \code{controllable.names = NULL}.

We have included in Figure~\ref{diag2} a diagram which schematically
describes the structure of the package, its different parts and
interconnections. 

\begin{figure}[t]
  \centering
  \includegraphics[width=14cm]{FunctionalDiagram1_1_letters}
  \caption{Schematic representation of the structure of the
    package.}\label{diag2} 
\end{figure}
...
When putting together the package, we have taken into account the fact
that 
many of the calculations implemented by the functions described above
can be computationally 
demanding and numerically unstable. For example, it's a well-known fact that
covariance 
matrices arising in Gaussian process likelihoods can be close
to singular even for relatively small sample sizes. As such, when
evaluating the likelihood we never invert matrices and instead perform 
the Cholesky decomposition (which is also useful to compute the
determinant) and solve triangular systems. In some situations, like
when performing predictions, we resort to the pivotal Cholesky
decomposition because the regular Cholesky decomposition tends to
break down. All the linear algebra calculations are
performed with the help of the the \proglang{BLAS} and
\proglang{LAPACK} numerical libraries, which
are known 
to be quite stable and accurate. The size of the field and model
datasets determines the size of the matrices that need to be
manipulated, and hence represent the biggest bottleneck when it comes
to the speed of the calculations. As we have already stressed, in order to
make the package as fast as possible we have implemented all the
computationally intensive computations in \proglang{C}. Nevertheless,
large sample sizes could certainly be an issue. So far, the package
has only been tested in problems where the sample sizes can be
considered small, say below 50.

\section{A real data example} \label{spotweld}
...
applied load (\code{load}). Electric current of certain magnitude
(\code{current}) is 
passed through 
...
Included in the package are two data sets, \code{spotweldfield} and
...
loading the package (\code{library("SAVE")}), the data frames can be
loaded using the commands 
\begin{Code}
R> data(spotweldfield, package = "SAVE")
\end{Code}
and 
\begin{Code}
R> data(spotweldmodel, package = "SAVE")
\end{Code}
...
We start this analysis by setting up the problem. This is accomplished
by creating
\code{sw}--- an object of the class 
\code{SAVE-class}--- using the function \code{SAVE}:
\begin{Code}
R> sw <- SAVE(response.name = "diameter", 
+             controllable.names = c("current", "load", "thickness"),
...
{\small\scalebox{0.75}{
\begin{tabular}{rcccc}
& {\tt data.frame} & Response ($y$) & Controllable ($\n x$) &
Calibration ($\n v$)\\ 
\hline
%& \multicolumn{3}{c}{Computer model names: {\tt spotweldmodel\$}}\\
Computer model names & {\tt spotweldmodel} & {\tt \$diameter} & {\tt \$current}, {\tt \$load}, {\tt \$thickness} & {\tt \$tuning}\\
%\hline
%& \multicolumn{3}{c}{Field names: {\tt spotweldfield\$}}\\
Field names & {\tt spotweldfield} & {\tt \$diameter} & {\tt \$current}, {\tt \$load}, {\tt \$thickness} & -\\
\end{tabular}
}
}
\end{center}
\caption{{\small Correspondence between the notation in Section 2 and
...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.2]{SW.pdf}
...
+                      n.thin = 2)
\end{Code}
We have now created a new object --- \code{swbayes} --- but 
instead we could have updated the object we had already created,
...
\begin{figure}[h]
  ...
  \caption{Posterior distribution of the calibration parameter
    \code{tuning} (spotweld example). The dotted line corresponds to the
    prior used.} 
  \label{cal}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Posterior distribution of the precisions (spotweld example). The dashed 
    vertical lines indicate the estimates of the parameters; the
    dash-dot lines indicate the priors used.}
  \label{prec}
\end{figure}

We can of course access the raw data. By running the
command \code{slotNames(swbayes)} we get a description of the
names of all the
slots of the object \code{swbayes}, and it is then clear how to obtain the
MCMC samples: \code{swbayes@mcmcsamples}.

After fitting the Bayesian model, we can finally produce predictions
of reality and also assess the quality of pure-model and
bias-corrected predictions of
...
R> xnew <- expand.grid(current = curr, load = load, thickness=g)
R>
R> set.seed(0)
R> valsw <- validate(object = swbayes, newdesign = xnew,
+                    calibration.value = "mean", n.burnin = 100)
\end{Code}
Above, we first construct the design of controllable inputs at which we want
...
current. As a remark, note that in the situation where \code{controllable.names = NULL}, 
the input \code{newdesign} must be left unspecified. 
This comment also applies when using the function \code{predictreality} in the same context.

Regarding the pure-model prediction, we are setting the
calibration parameters at the corresponding posterior mean. Note that we could set 
the calibration parameters at a particular value if we chose to. We would do it by 
specifying the input \code{calibration.value} to be a named \code{data.frame} containing those values. 
...
\begin{figure}[h]
  ...
  \caption{Default validation plot for the spotweld example. The plot
...
\begin{figure}[h]
  ...
  \caption{Customized validation plot for the spotweld example. Here
    we can see the pure-model 
    prediction 
    ...
    and the circles represent the observed field data corresponding to
    that particular combination of controllable inputs.}
  \label{valplotcustom}
\end{figure}

A referee has pointed out how easy it is to incorporate
further insight into the statistical analysis of a particular
problem using the package. As an example, we can easily incorporate  a
linear trend in 
the mean function for the emulator. This would be accomplished simply
by starting the analysis with the command
\begin{Code}
R> sw2 <- SAVE(response.name = "diameter", 
+              controllable.names = c("current", "load", "thickness"),
+              calibration.names = "tuning", field.data = spotweldfield,
+              model.data = spotweldmodel, 
+              mean.formula = ~ current + load + thickness),
+              bestguess = list(tuning = 4.0))
\end{Code}
which produces an new \code{SAVE-class} object, \code{sw2}, and then we can
proceed as before.
...
\code{diameter} 
varies with \code{current}. One might assess this variation by
looking at the derivative of \code{diameter} with respect to
\code{current}. Let's 
do that for \code{thickness = 1} and \code{load = 4}. We start by
predicting reality 
...
obtain the 
corresponding derivatives with respect to \code{current} of each of these
...
\begin{figure}[h]
  ...
  \caption{Bias-corrected prediction of the derivative of \code{diameter}
...
\code{tuning}, say, \code{tuning}=3.2. This allows us to produce the
pure-model estimate of the 
derivative:
\begin{Code}
R> u <- 3.2; load <- 4; g <- 1; curr <- seq(from=20,to=30,length=80)
R> xnewpure <- expand.grid(current = curr, load = load, thickness = g, 
+                          tuning = u)
R>
R> set.seed(0)
R> pmsw <- predictcode(object = swbayes, newdesign = xnewpure,
+                      n.iter = 20000)
R>
...
\section{A simulated example}\label{synth}
In this section, we are going to illustrate the use of the package in
a simulated data example: model and reality are known, and so is the
true value of the calibration parameter and of the field data
precision. Essentially the same setup has already been utilized in
\citet{Baya:2007} and in \citet{baya:2010} to illustrate similar
points.

In this example, we have a calibration parameter $v$ whose true value
is fixed at $u=1.7$. The controllable input is $x$; reality is given by 
\begin{equation}
y^R(x)= 3.5\ \exp(-u\ x) + 1.5\ .
\end{equation}
The computer model is given by
\begin{equation}
y^M(x,v) = 5\ \exp(-v\ x)
\end{equation}
but we are going to treat it as we would a computationally expensive
model: to obtain
the model 
data, we evaluate it at a $6\times 6$ equally-spaced grid 
on the $(x,v)$-space, which is set at $[0,3]\times[0.5,2]$. We use
these data, available with the package as 
\code{synthmodel}, to produce the emulator; to load the data frame, use
the command
\begin{Code}
R> data(synthmodel, package = "SAVE")
\end{Code}


The field data consists of 3 replicates at each point in an
equally-spaced grid of 
10 points for $x$ spanning the interval $[0.11,3.01]$. The measurement
error is $\textrm{N}(0,0.3^2)$, that is $\lambda^F=1/0.3^2\approx
11.11$. These data are also available with the package; use the
following command to load the corresponding data frame:
\begin{Code}
R> data(synthfield, package = "SAVE")
\end{Code}

The \proglang{R} code to produce the \code{SAVE} object associated
with this analysis is
\begin{Code}
R> synth <- SAVE(response.name = "y", controllable.names = "x",
+                calibration.names = "v", field.data = synthfield,
+                model.data = synthmodel, mean.formula = ~1+x, 
+                bestguess = list(v = 1.5))
\end{Code}
and the MCMC samples were obtained with
\begin{Code}
R > set.seed(0)
R > synth <- bayesfit(object = synth, 
+                     prior = uniform(var.name = "v", lower = 0, 
+                     upper = 3), n.iter = 20000)
\end{Code}
The plot of the posterior distribution on the calibration parameter
can be found in Figure~\ref{synthu}, and the posterior on the
precisions is in Figure~\ref{synthprec}. Note how the posteriors
include the true values of $v$ and $\lambda^F$. 

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.3]{synthu.pdf}
  \caption{Posterior distribution on the calibration input for the
    simulated example.}
  \label{synthu}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.3]{synthprec.pdf}
  \caption{Posterior distribution on the precisions for the
    simulated example.}
  \label{synthprec}
\end{figure}

We next produce the bias-corrected prediction of reality at
an equally-spaced grid of 25 points for $x$ in the interval
$[0.05,3.05]$. For that, we have used the code
\begin{Code}
R> xnew <- data.frame(x = seq(from = 0.05, to = 3.05, length = 25))
R> valsynth <- validate(object = synth, newdesign = xnew, n.burnin = 100)
\end{Code}
The bias-corrected prediction of reality is the solid black line in
Figure~\ref{synthval}. The dashed lines correspond to the tolerance
bars, and the red solid line is the actual curve we are trying to
estimate; the stars represent the simulated field data.

Using the fact that we know $y^M$, it's easy to find the least squares
estimate of $u$, i.e., the value of $v$ that minimizes the sum of the
squared distance between the field observations and the model
output. That estimate here turns out to be $\tilde{u}=0.63$. This is a
common strategy in calibration: not including a bias 
term. It has been argued extensively, starting with
\citet{Kenn:OHag:2001} and \citet{Crai:1997}, that this approach
often leads to over-fitting 
and to unreasonable estimates for the calibration parameter. In this
example, we again see evidence of that. In Figure~\ref{synthval}, the
green curve corresponds the true model output evaluated at
$\tilde{u}$. The blue curve represents the true model output evaluated
at the posterior mean, which turns out to be $\hat{u}=1.58$. The
inclusion of the bias prevents over-fitting and allows for a more
reasonable estimate of the calibration parameter. Additionally, it's
clear that the bias-corrected prediction is a much better estimate of
the true reality curve than the pure-model prediction (computer model
evaluated at the posterior mean) and than the computer model evaluated at
the least squares estimate. 

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.3]{synthval.pdf}
  \caption{Bias-corrected prediction for the simulated example. The
    solid black line corresponds to the bias-corrected prediction, the
    dashed lines represent the tolerance bars, and the stars are the
    simulated field observations. The red line corresponds to the
    actual reality curve, which is known in this case. The blue and
    green curves correspond, respectively, to the computer model
    evaluated at the posterior mean and at the least squares estimate
...
Statistical Sciences (NISS), and at the time their research was
...
updated mean and covariance functions given respectively by
\begin{align}
& \E[y^M(\n{z}) \mid \n{y}^M,
\n{\theta}^L, \n{\theta}^M] =\n{\Psi}^\top(\n{z}) \,
\n{\theta}^L +
\n{r}_z^\top(\n{\Gamma}^M)^{-1}(\n{y}^M-\n{X}\n{\theta}^L) \label{meanapp}\\
&\COV[\, y^M(\n{z}), y^M(\n{z}^{\star})
\mid \n{y}^M, \n{\theta}^L, \n{\theta}^M] =
\frac{1}{\lambda^M} \, c^M(\n{z},\n{z}^{\star}) -
\n{r}_z^\top(\n{\Gamma}^M)^{-1}\n{r}_{z^{\star}},
\end{align}
where $\n{r}_z^\top = \frac{1}{\lambda^M} \ (c^M(\n{z},
\n{z}_1),\ldots,c^M(\n{z},\n{z}_N))$,
$\n{X}$ is the matrix 
with rows $\n{\Psi}^\top(\n{z}_1),\ldots, \n{\Psi}^\top(\n{z}_N)$, and
$\n{\Gamma}^M$ is
a matrix with $(i,j)$ entry given by $c^M(\n{z}_i,\n{z}_j)/\lambda^M$
...
argument \code{bestguess} in function \code{SAVE}. Then, using the
...
%\bibliography{/Users/gonzalogarcia-donato/Mywork/bibliografia/Computermodels,
%/Users/gonzalogarcia-donato/Mywork/bibliografia/biblio}
